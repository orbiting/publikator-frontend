...
test('rules.remove', assert => {
  ...
  assert.equal(
    remove(value.change(), value.document, nodeToRemove).value
      .document.nodes.size,
    2,
  ...
  assert.equal(
    remove(value.change(), value.document, nodesToRemove).value
      .document.nodes.size,
    1,
...
test('rules.insertAt', assert => {
  ...
  assert.equal(
    insertAfterSecond(value.change(), value.document).value.document
      .nodes.size,
    5,
    'reducers can return a list of nodes to insert',
  )

  const updatedState = insertAfterSecond(
    value.change(),
    value.document,
  ).value
...
test('rules.update', assert => {
  ...
  assert.equal(
    nodeA.data.get('foo') === 'bar' &&
      nodeB.data.get('foo') === 'bar',
    true,
