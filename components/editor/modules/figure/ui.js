      ...
      return (
        <div>
          {value.blocks
            ...
            .filter(
              (block, index, all) => all.indexOf(block) === index,
            )
            .map((block, i) => {
                ...
                <div key={`figure-${i}`}>
                  ...
                  {captionRight && (
                    <MetaForm
                      data={Map({
                        captionRight:
                          captionBlock.data.get('captionRight') ||
                          false,
                      })}
                  ...
                  {!!applicableSizes.length && (
                    <p style={{ margin: '10px 0' }}>
                      ...
                      {applicableSizes.map((size, i) => {
                        ...
                        if (size.unwrap) {
                          checked =
                            checked && parent.kind === 'document'
                        }
                        if (size.wrap) {
                          checked =
                            checked && parent.type === size.wrap
                        }
                        ...
                        return [
                          <Radio
                            ...
                            onChange={event => {
                              ...
                              if (size.unwrap) {
                                for (
                                  let i = value.document.getDepth(
                                    block.key,
                                  );
                                  i > 1;
                                  i--
                                ) {
                                  change = change.unwrapNodeByKey(
                                    block.key,
                                  )
                                }
                              } else if (
                  ...
                  {value.document.data.get('gallery') === true && (
                    <p>
                      <Checkbox
                        checked={
                          block.data.get('excludeFromGallery') ===
                          true
                        }
                        onChange={onInputChange(block)(
                          'excludeFromGallery',
                        )}
                      >
