    ...
    <UIForm>
      ...
      {options.includes('textPosition') && (
        <Dropdown
          ...
          onChange={({ value }) =>
            onChange('textPosition', null, value)
          }
        />
      )}
...
export const TeaserForm = ({ subModuleResolver, ...options }) => {
  ...
  return createPropertyForm({
    ...
    const handlerFactory = createOnFieldChange(
      change => {
        const newTeaser = change.value.document.getDescendant(
          teaser.key,
        )
    ...
    const handleRepo = repoData => {
      ...
      return onChange(
        value.change().replaceNodeByKey(teaser.key, newNode),
      )
    }
    ...
    const handleTypeChange = () => {
      return onChange(
        value.change().replaceNodeByKey(
          teaser.key,
          Block.create({
            data: teaser.data.set(
              'onlyImage',
              !teaser.data.get('onlyImage'),
            ),
    ...
    const isTile = !!options.rule.editorOptions.teaserType.match(
      /tile/i,
    )
    ...
    return (
      <div>
        <Label>
          {options.rule.editorOptions.formTitle || 'Teaser'}
        </Label>
        <br />
        {isTile && (
          <>
        ...
        {!options.rule.editorOptions.formOptions.includes(
          'noAdapt',
        ) && (
          <RepoSearch
            value={null}
...
export const TeaserInlineUI = ({
  ...
  const moveHandler = dir => event => {
    event.preventDefault()
    editor.change(t =>
      t.moveNodeByKey(node.key, parentNode.key, index + dir),
    )
  }

  const endIndex = parentNode.nodes.findIndex(
    n => n.data.get('id') === 'end',
  )
